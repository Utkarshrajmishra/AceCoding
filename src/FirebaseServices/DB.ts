...
class DB {
  ...
  async UploadUserInfo(email: string, imageUrl: string, name: string) {
    ...
    try {
      const User = await setDoc(doc(databaseRef, "UserInfo", email), UserInfo);
       if(User!=null) 
  ...
  async UploadDates(email: string) {
    try {
        ...
        const Dates= await setDoc(doc(databaseRef,"Dates",email),next30Days);
        if(Dates!=null)
            return {storage:Dates, status:false}
    } catch (error) {
      return { storage: error, status: false };
    }
  }
